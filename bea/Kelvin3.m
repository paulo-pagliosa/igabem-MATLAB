...
methods (Static)
  ...
  function [D, S] = computeDS(p, q, N, m)
    ...
    S(1, 1, 1) = c * (m.c4 * (3 * N(1) * r11 + 2 * N(1)) - N(1) * m.c6 ...
      + 3 * rN * (m.c4 * r(1) + 2 * m.nu * r(1) - 5 * r(1) ^ 3) ...
      + 6 * N(1) * m.nu * r11);
    S(1, 2, 1) = c * (m.c4 * (N(2) + 3 * N(1) * r12) ...
      + 3 * rN * (m.nu * r(2) - 5 * r(2) * r11) ...
      + 3 * m.nu * (N(2) * r11 + N(1) * r(2) * r(1)));
    S(1, 3, 1) = c * (m.c4 * (N(3) + 3 * N(1) * r13) ...
      + 3 * rN * (m.nu * r(3) - 5 * r(3) * r11) ...
      + 3 * m.nu * (N(3) * r11 + N(1) * r(3) * r(1)));
    S(2, 1, 1) = S(1, 2, 1);
    S(2, 2, 1) = c * (3 * rN * (m.c4 * r(1) - 5 * r(1) * r22) - N(1) * m.c6 ...
      + 3 * N(1) * m.c4 * r22 + 6 * N(2) * m.nu * r12);
    S(2, 3, 1) = c * (3 * m.nu * (N(2) * r13 + N(3) * r12) ...
      - 15 * r12 * r(3) * rN + 3 * N(1) * m.c4 * r23);
    S(3, 1, 1) = S(1, 3, 1);
    S(3, 2, 1) = S(2, 3, 1);
    S(3, 3, 1) = c * (3 * rN * (m.c4 * r(1) - 5 * r(1) * r33) - N(1) * m.c6 ...
      + 3 * N(1) * m.c4 * r33 + 6 * N(3) * m.nu * r13);
    S(1, 1, 2) = c * (3 * rN * (m.c4 * r(2) - 5 * r(2) * r11) - N(2) * m.c6 ...
      + 3 * N(2) * m.c4 * r11 + 6 * N(1) * m.nu * r12);
    S(1, 2, 2) = c * (m.c4 * (N(1) + 3 * N(2) * r12) ...
      + 3 * rN * (m.nu * r(1) - 5 * r(1) * r22) ...
      + 3 * m.nu * (N(1) * r22 + N(2) * r12));
    S(1, 3, 2) = c * (3 * m.nu * (N(1) * r23 + N(3) * r12) ...
      - 15 * r12 * r(3) * rN + 3 * N(2) * m.c4 * r13);
    S(2, 1, 2) = S(1, 2, 2);
    S(2, 2, 2) = c * (m.c4 * (3 * N(2) * r22 + 2 * N(2)) - N(2) * m.c6 ...
      + 3 * rN * (m.c4 * r(2) + 2 * m.nu * r(2) - 5 * r(2) ^ 3) ...
      + 6 * N(2) * m.nu * r22);
    S(2, 3, 2) = c * (m.c4 * (N(3) + 3 * N(2) * r23) ...
      + 3 * rN * (m.nu * r(3) - 5 * r(3) * r22) ...
      + 3 * m.nu * (N(3) * r22 + N(2) * r(3) * r(2)));
    S(3, 1, 2) = S(1, 3, 2);
    S(3, 2, 2) = S(2, 3, 2);
    S(3, 3, 2) = c * (3 * rN * (m.c4 * r(2) - 5 * r(2) * r33) - N(2) * m.c6 ...
      + 3 * N(2) * m.c4 * r33 + 6 * N(3) * m.nu * r23);
    S(1, 1, 3) = c * (3 * rN * (m.c4 * r(3) - 5 * r(3) * r11) - N(3) * m.c6 ...
      + 3 * N(3) * m.c4 * r11 + 6 * N(1) * m.nu * r13);
    S(1, 2, 3) = c * (3 * m.nu * (N(1) * r23 + N(2) * r13) ...
      - 15 * r12 * r(3) * rN + 3 * N(3) * m.c4 * r12);
    S(1, 3, 3) = c * (m.c4 * (N(1) + 3 * N(3) * r13) ...
      + 3 * rN * (m.nu * r(1) - 5 * r(1) * r33) ...
      + 3 * m.nu * (N(1) * r33 + N(3) * r13));
    S(2, 1, 3) = S(1, 2, 3);
    S(2, 2, 3) = c * (3 * rN * (m.c4 * r(3) - 5 * r(3) * r22) - N(3) * m.c6 ...
      + 3 * N(3) * m.c4 * r22 + 6 * N(2) * m.nu * r23);
    S(2, 3, 3) = c * (m.c4 * (N(2) + 3 * N(3) * r23) ...
      + 3 * rN * (m.nu * r(2) - 5 * r(2) * r33) ...
      + 3 * m.nu * (N(2) * r33 + N(3) * r23));
    S(3, 1, 3) = S(1, 3, 3);
    S(3, 2, 3) = S(2, 3, 3);
    S(3, 3, 3) = c * (m.c4 * (3 * N(3) * r33 + 2 * N(3)) - N(3) * m.c6 ...
      + 3 * rN * (m.c4 * r(3) + 2 * m.nu * r(3) - 5 * r(3) ^ 3) ...
      + 6 * N(3) * m.nu * r33);
  end
end
