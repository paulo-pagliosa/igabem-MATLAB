...
% See also: Mesh
  ...
  fprintf(file, ['\n# Several rows per element. The first row specifies ' ...
    'the\n# type id, the shell index (0: outer shell), the number\n' ...
    '# of nodes (s), and the matrix flag (f) of the element.\n' ...
    '# The next two rows contain the node id and node region\n' ...
    '# lists of the element, respectively. For f != 0, those\n' ...
    '# are followed by s rows containing the coefficients of\n' ...
    '# the element matrix\n']);
  for i = 1:ne
    element = mesh.elements(i);
    C = element.shapeFunction.C;
